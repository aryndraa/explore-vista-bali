...
<div class="-space-y-5 group overflow-hidden isolate">
    ...
    <div class="cursor-pointer relative -z-10">
        ...
        <img class="-z-10 w-full h-56 object-cover object-center transition group-hover:scale-105 rounded-t-xl"
            src="{{ $img }}" alt="{{ $title }}">
    </div>
        ...
        <div class="flex flex-wrap justify-start gap-4 font-inter [&_p]:font-semibold mb-4">
            @if ($startTime)
                <div class="flex items-center gap-0.5 sm:gap-2 whitespace-nowrap">
                    ...
                    <p class="text-md">{{ \Carbon\Carbon::parse($startTime)->format('h:i A') }}</p>
                </div>
            @endif
            ...
            @if ($price)
                <div class="flex items-center gap-0.5 sm:gap-2 whitespace-nowrap">
                    ...
                    <p class="text-md">${{ number_format($price, 2) }}</p>
                </div>
            @endif
        </div>

        <a
            wire:key="wave-button-{{ $id }}"
            href="{{ route('services.package-detail', ['package' => $id]) }}"
            class="text-lg w-full flex justify-center items-center font-semibold py-2 text-center bg-cst-green-400 rounded-sm text-cst-yellow-400">
